 ...
 */
export function addKhedmaStatus(req: Request, res: Response, next: NextFunction) {
 ...
 * @param next NextFunction
 */
export function addKhedmaStatuses(req: Request, res: Response, next: NextFunction) {
  insertMany(req.body)
    .then((done: any) => {
      handleResponse(res, 201, true, "Added Successfully");
    })
...
/**
 * Get All Khedma Statuses
 * @param req Request
 * @param res Response
 * @param next NextFunction
 */
export function getKhedmaStatuses(req: Request, res: Response, next: NextFunction) {
  find()
    .then((results: any) => {
...
export function getById(req: Request, res: Response, next: NextFunction) {
  findById(parseInt(req.params.id))
    .then((result: any) => {
      if (!result) return next(new HttpException(404, "Khedma Status Not Found"));
 ...
 * @param next NextFunction
 */
export function editKhedmaStatus(req: Request, res: Response, next: NextFunction) {
  updateKhedmaStatus(
...
/**
 * Remove Khedma Status
 * @param req Request
 * @param res Response
 * @param next NextFunction
 */
export function removeKhedmaStatus(req: Request, res: Response, next: NextFunction) {
  deleteKhedmaStatus(parseInt(req.params.id))
    .then((result) => {
      if (!result) return next(new HttpException(404, "Khedma Status Not Found"));
...
const errMessage = centralizedErrMsgs("Khedma Status");
