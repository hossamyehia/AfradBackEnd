...
export default function routePermission(
  ...
  if (req.baseUrl == "/api/user" || req.baseUrl == "/api/user/register")
    index = 0;
  else if (req.baseUrl == "/api/role") index = 1;
  else if (req.baseUrl == "/api/dataentry") index = 2;
  else if (req.baseUrl == "/api/dobatsaf") index = 3;
  else if (req.baseUrl == "/api/dabetinfo") index = 4;
